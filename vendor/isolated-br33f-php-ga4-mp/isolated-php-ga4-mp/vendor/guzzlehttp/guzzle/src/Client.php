...
use Isolated\Blue_Media\Isolated_Php_ga4_mp\GuzzleHttp\Handler\CurlShareHandleState;
...
class Client implements ClientInterface, \Isolated\Blue_Media\Isolated_Php_ga4_mp\Psr\Http\Client\ClientInterface
{
    ...
    {
        \trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Passing %s to request option "%s" is deprecated; guzzlehttp/guzzle 8.0 requires %s.', \get_debug_type($value), $option, $expected);
    }
    /**
        ...
        if (!empty($options['auth']) && \is_array($options['auth'])) {
            ...
            switch ($type) {
                ...
                case 'digest':
                    ...
                    break;
                case 'ntlm':
                    \trigger_deprecation('guzzlehttp/guzzle', '7.12', 'Passing "ntlm" as the built-in auth type is deprecated; guzzlehttp/guzzle 8.0 will no longer apply NTLM through the "auth" request option. Configure NTLM with cURL HTTP authentication options instead.');
