...
$card_widget_data = isset( $card_widget_data ) && is_array( $card_widget_data )
	? $card_widget_data
	: [];

$bm_cw_service_id = isset( $card_widget_data['service_id'] ) ? (string) $card_widget_data['service_id'] : '';
$bm_cw_amount     = isset( $card_widget_data['amount'] ) ? (string) $card_widget_data['amount'] : '0';
$bm_cw_currency   = isset( $card_widget_data['currency'] ) ? (string) $card_widget_data['currency'] : 'PLN';
$bm_cw_language   = isset( $card_widget_data['language'] ) ? (string) $card_widget_data['language'] : 'pl';
$bm_cw_cards      = isset( $card_widget_data['cards_domain'] ) ? (string) $card_widget_data['cards_domain'] : '';
$bm_cw_wait_msg   = __( 'Enter your card details in the form above to place the order.', 'bm-woocommerce' );
?>

<div
	class="bm-card-widget bm-card-widget--classic"
	id="bm-card-widget"
	data-service-id="<?php echo esc_attr( $bm_cw_service_id ); ?>"
	data-amount="<?php echo esc_attr( $bm_cw_amount ); ?>"
	data-currency="<?php echo esc_attr( $bm_cw_currency ); ?>"
	data-language="<?php echo esc_attr( $bm_cw_language ); ?>"
	data-cards-domain="<?php echo esc_attr( $bm_cw_cards ); ?>"
	data-wait-msg="<?php echo esc_attr( $bm_cw_wait_msg ); ?>"
>
	<p class="bm-card-widget__help"><?php echo esc_html( $bm_cw_wait_msg ); ?></p>
	<input
		type="hidden"
	...
	<div class="bm-card-widget__iframe-wrapper" id="bm-card-widget-iframe-wrapper">
		<iframe
			...
			title="<?php echo esc_attr__( 'Card payment form', 'bm-woocommerce' ); ?>"
		></iframe>
	</div>
</div>
<?php
// Classic checkout: attach after Autopay widget SDK (footer) so WidgetConnection exists; once per request.
static $bm_autopay_card_widget_classic_js_added = false;
if ( ! $bm_autopay_card_widget_classic_js_added && wp_script_is( 'autopay_card_widget', 'enqueued' ) ) {
	$bm_autopay_card_widget_classic_js_added = true;
	$bm_cw_inline_js = <<<'JS'
...
JS;
	wp_add_inline_script( 'autopay_card_widget', $bm_cw_inline_js, 'after' );
}
