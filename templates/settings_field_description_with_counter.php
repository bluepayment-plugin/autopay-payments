...
$max_length = isset( $template_args['max_length'] ) ? (int) $template_args['max_length'] : 500;
$value      = (string) $wc_settings_api->get_option( $key );
$tip_url    = isset( $template_args['tip_url'] ) ? $template_args['tip_url'] : '';
$tip_label  = isset( $template_args['tip_url_label'] ) ? $template_args['tip_url_label'] : '';
?>

<tr valign="top" class="<?php echo esc_attr( $field_key ); ?>-tr autopay-description-field <?php esc_attr_e( implode( ' ', $tr_classes ) ) ?>">
	<th scope="row" class="titledesc">
		<label for="<?php echo esc_attr( $field_key ); ?>"><?php echo wp_kses_post( $data['title'] ); ?></label>
		<?php if ( ! empty( $tip_url ) ): ?>
			<a href="<?php echo esc_url( $tip_url ); ?>" target="_blank" rel="noopener noreferrer"><?php echo esc_html( $tip_label ); ?></a>
		<?php endif; ?>
		<span class="autopay-counter" data-for="<?php echo esc_attr( $field_key ); ?>">
				<?php echo esc_html__( 'max. 500 characters', 'bm-woocommerce' ); ?>
			</span>
	</th>
	<td class="forminp">
		<fieldset>
			<textarea
				class="input-text large-text <?php echo esc_attr( $data['class'] ); ?>"
				name="<?php echo esc_attr( $field_key ); ?>"
				id="<?php echo esc_attr( $field_key ); ?>"
				style="<?php echo esc_attr( $data['css'] ); ?>;height:84px"
				maxlength="<?php echo esc_attr( $max_length ); ?>"
				aria-describedby="<?php echo esc_attr( $field_key ); ?>-help <?php echo esc_attr( $field_key ); ?>-error"><?php echo esc_textarea( $value ); ?></textarea>
